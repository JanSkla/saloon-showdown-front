...
const MainCamera = () => {
  ...
  useFrame(({ clock }) => {
    ...
    cameraRef.current.rotation.x = currentRot.current[0] - (-Math.sign(cursor.current.y) + Math.sign(cursor.current.y) * Math.pow(cursor.current.y - Math.sign(cursor.current.y), 2)) * MULTIPLIER;
    cameraRef.current.rotation.y = currentRot.current[1] - (-Math.sign(cursor.current.x) + Math.sign(cursor.current.x) * Math.pow(cursor.current.x - Math.sign(cursor.current.x), 2)) * MULTIPLIER;
  })
